...
import { ChevronLeft, Bot, Settings, Activity, LogOut, Info, AlertTriangle, Edit3, X, Trash2 } from 'lucide-react';
import { supabase } from '../supabaseClient';
import emailjs from '@emailjs/browser'; // Import EmailJS

export default function Navbar({ onBack, teamName, projectId, onOpenAbout, userName }) {
  ...
  const handleReportError = () => {
    const issue = window.prompt("Describe the error/issue in detail:");
    ...
    const templateParams = {
      from_name: userName || "Anonymous User",
  ...
  const handleUpdateName = async () => {
    ...
    const { error } = await supabase.from('projects').update({ team_name: newName }).eq('id', projectId);
    if (!error) {
      setIsEditing(false);
      setShowSettings(false);
    }
  ...
  const handleTerminateHackathon = async () => {
    const confirmMessage = `WARNING: This will permanently delete "${teamName}" and all associated tasks/messages. Type "TERMINATE" to confirm:`;
    ...
    if (confirmation === "TERMINATE") {
      try {
        ...
        console.error("Error terminating project:", error.message);
      }
    }
  };
          ...
          <div style={modalContent}>
            ...
            <div style={modalBody}>
              <div style={settingItem}>
                <div style={settingInfo}><Edit3 size={16} color="#888" /><span>Team Name</span></div>
                {isEditing ? (
                  <div style={{display:'flex', gap:'5px'}}>
                    <input style={smallInput} value={newName} onChange={(e) => setNewName(e.target.value)} />
                    <button onClick={handleUpdateName} style={saveBtn}>Save</button>
                  </div>
                ) : ( <button onClick={() => setIsEditing(true)} style={editBtn}>Change</button> )}
              </div>

              {/* MODIFIED: Calls handleReportError */}
              <button onClick={handleReportError} style={settingItemBtn}>
                <div style={settingInfo}><AlertTriangle size={16} color="#ffab00" /><span>Report Error</span></div>
              </button>
              ...
              <button onClick={handleTerminateHackathon} style={{...settingItemBtn, color: '#ff4b2b', background: 'rgba(255, 75, 43, 0.05)'}}>
                <div style={settingInfo}><Trash2 size={16} /><span>Terminate Hackathon</span></div>
              </button>

              <button onClick={onBack} style={settingItemBtn}>
                <div style={settingInfo}><LogOut size={16} color="#888" /><span>Exit Workspace</span></div>
              </button>
            </div>
          </div>
        </div>
