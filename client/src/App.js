...
import About from './pages/About'; // Import your new About component
...
function App() {
  ...
  useEffect(() => {
    if (projectId) {
      ...
      // Only set to dashboard if we aren't already on the about page
  ...
  useEffect(() => {
    ...
    const fetchInitialMessages = async () => {
      const { data } = await supabase.from('projects').select('messages').eq('id', projectId).single();
  ...
  return (
    <div className="App" style={appStyle}>
      {/* Navbar shows on Dashboard OR About page as long as there is a project */}
      {projectId && view !== 'landing' && view !== 'setup' && (
        <Navbar 
          onBack={handleBack} 
          teamName={teamName || "Loading..."} 
          projectId={projectId}
          onOpenAbout={() => setView('about')} // Prop to switch to About view
        />
      )}
      ...
      <main style={mainWrapper}>
        {view === 'landing' && (
          <div style={landingCard}>
            ...
            <button onClick={() => userName.trim() ? setView('setup') : alert("Enter name!")} style={mainBtn}>Create New Team</button>
        ...
        <aside style={chatBox}>
          ...
          <div style={chatMessages}>
            {messages.length > 0 ? messages.map((m, i) => (
              <div key={m.id || i} style={{...msg, alignSelf: m.sender === userName ? 'flex-end' : 'flex-start', background: m.sender === userName ? '#1e1e1e' : '#0a0a0a', border: m.sender === userName ? '1px solid #333' : '1px solid #1a1a1a'}}>
          ...
          <div style={chatInputArea}>
            <div style={{display:'flex', gap:'8px'}}>
              <input 
                ...
                placeholder="Type..." 
              />
              <button onClick={handleSendMessage} style={sendBtn}><Send size={14}/></button>
            </div>
          </div>
        </aside>
      )}
      ...
      {view === 'dashboard' && (
        <>
          <div style={floatingInfoPanel}>
             <div style={infoItem} onClick={() => {navigator.clipboard.writeText(projectId); alert("ID Copied!")}}>
                <Copy size={12} />
...
// ... styles remain same ...
const appStyle = { background: '#050505', minHeight: '100vh', color: 'white', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' };
const mainWrapper = { flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', overflow: 'hidden' };
const floatingChatToggle = { position: 'fixed', bottom: '25px', right: '25px', width: '60px', height: '60px', borderRadius: '50%', background: '#3ecf8e', color: '#000', border: 'none', boxShadow: '0 8px 24px rgba(62, 207, 142, 0.3)', cursor: 'pointer', zIndex: 1001, display: 'flex', alignItems: 'center', justifyContent: 'center' };
...
const logoLarge = { fontSize: '3rem', marginBottom: '20px' };
...
const mainBtn = { width: '100%', padding: '16px', background: '#3ecf8e', color: '#000', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: 'bold' };
const dividerText = { margin: '25px 0', color: '#333', fontSize: '0.8rem' };
const secBtn = { width: '100%', padding: '16px', background: 'transparent', color: '#3ecf8e', border: '1px solid #1a1a1a', borderRadius: '12px', cursor: 'pointer' };
...
const sendBtn = { background: '#3ecf8e', color: '#020617', border: 'none', borderRadius: '6px', padding: '0 12px', cursor: 'pointer' };
