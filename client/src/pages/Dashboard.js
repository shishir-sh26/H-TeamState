...
import { Plus, Trash2, Play, Pause, Clock, Edit2, RotateCcw } from 'lucide-react';
...
export default function Dashboard({ projectId, onBack, userName, setGlobalTeamName }) {
  ...
  const [priority, setPriority] = useState("Medium"); // Fixed: Now used in the Add Task UI
  ...
  const resetTimer = async () => {
    if(window.confirm("Reset timer?")) {
      await supabase.from('projects').update({ end_time: null, timer_status: 'stopped' }).eq('id', projectId);
    }
  };
  ...
  const handleAddTask = async () => {
    ...
    const updated = [...(project.tasks || []), { 
      ...
      status: "pending", 
      priority: priority // Fixed: Uses the priority state
    }];
                    ...
                    <div style={timerActions}>
                      {(!project.end_time || project.timer_status === 'stopped') ? (
                        ...
                        <div style={{display:'flex', gap:'8px'}}>
                          ...
                          <button onClick={resetTimer} style={btnIcon}><RotateCcw size={18} color="#ff4b2b"/></button>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div style={taskListContainer}>
                {project.tasks?.map((t, i) => (
                  <div key={i} style={{...taskRow, borderLeft: t.priority === 'High' ? '4px solid #ef4444' : '4px solid #3ecf8e'}}>
                    ...
                    <button onClick={() => deleteTask(i)} style={deleteBtn}><Trash2 size={16} /></button>
                  </div>
                ))}
              </div>
            </div>
...
// --- STYLES ---
const dashboardWrapper = { display: 'flex', background: '#020202', height: '100vh', width: '100vw', color: '#f8fafc', overflow: 'hidden' };
